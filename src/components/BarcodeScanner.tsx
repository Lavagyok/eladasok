...
const BarcodeScanner: React.FC<BarcodeScannerProps> = ({
  ...
  const handleScan = useCallback(async (barcode: string) => {
    ...
    if (result) {
      setApiResult(result);
      const nameParts = [result.brand, result.name].filter(Boolean);
      setFormData({
        ...EMPTY_FORM,
        name: result.name,
  ...
  const handleSaveProduct = () => {
    setSaving(true);
    const productData = {
      name: formData.name || `Termék (${scannedBarcode})`,
  ...
  const handleAssignBarcode = () => {
    if (foundProduct) {
      onUpdateProduct(foundProduct.id, { barcode: scannedBarcode });
    }
    closeModal();
  };
                ...
                <div className="p-5 space-y-4">
                  ...
                  {scanState === 'not-found' && (
                    <div className="bg-yellow-900/20 border border-yellow-600/30 rounded-xl p-3">
                      <p className="text-xs text-yellow-400 font-medium mb-1">Vonalkód</p>
                      <p className="text-white font-mono">{scannedBarcode}</p>
                      <p className="text-gray-400 text-xs mt-1">Nem találtunk egyező terméket. Add hozzá manuálisan!</p>
                    </div>
                  )}
